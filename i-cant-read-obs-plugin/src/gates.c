...
struct GatePipeline {
	...
	GLuint tex_lap, tex_gray_ocr;       /* GL_R32F */
	GLuint tex_mask_cur, tex_mask_prev; /* GL_R8   */
	GLuint tex_mask_dil, tex_mask_ocr;  /* GL_R8   */
	...
	bool has_ocr;  // Updated on successful ocr
	float norm_lap_var_thr;
	float norm_diff_thr;
	float norm_change_thr;
...
static void swap_prev(GatePipeline *p)
{
	...
	p->tex_gray_cur = tmp;
	{
		GLuint eff_cur = ((p->cfg.modes & LAP_MASK_BIT) != 0) ? p->tex_mask_dil : p->tex_mask_cur;
		tmp = p->tex_mask_prev;
		p->tex_mask_prev = eff_cur;
		if ((p->cfg.modes & LAP_MASK_BIT) != 0)
			p->tex_mask_dil = tmp;
		else
			p->tex_mask_cur = tmp;
	}
...
static void swap_ocr(GatePipeline *p)
{
	GLuint tmp;
	tmp = p->tex_gray_ocr;
	p->tex_gray_ocr = p->tex_gray_cur;
	p->tex_gray_cur = tmp;
	{
		GLuint eff_cur = ((p->cfg.modes & LAP_MASK_BIT) != 0) ? p->tex_mask_dil : p->tex_mask_cur;
		tmp = p->tex_mask_ocr;
		p->tex_mask_ocr = eff_cur;
		if ((p->cfg.modes & LAP_MASK_BIT) != 0)
			p->tex_mask_dil = tmp;
		else
			p->tex_mask_cur = tmp;
...
static void gate_run_lap(GatePipeline *p, unsigned int gl_tex_id)
{
	...
	glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 0, p->ssbo_lap_partial);
	{
		struct {
			...
			int modes;
			int pad[3]; /* explicit: pad to 32 bytes (2×vec4) */
		} u = {(int)p->width, (int)p->height, p->norm_lap_edge, p->norm_black_thr, p->cfg.modes, {0, 0, 0}};
...
static void gate_run_lap_carve(GatePipeline *p)
{
	...
	glBindImageTexture(0, p->tex_mask_dil, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_R8);
	{
		struct {
			int radius, w, h, pad;
		} u = {p->cfg.lap_dilate_kernel > 1 ? p->cfg.lap_dilate_kernel / 2 : 1, (int)p->width, (int)p->height,
		       0};
...
static void gate_run_lap_reduce(GatePipeline *p, bool *out_result, float *out_lap_var)
{
	...
	glMemoryBarrier(GL_BUFFER_UPDATE_BARRIER_BIT);
	{
		...
		if (lap_var <= p->norm_lap_var_thr) {
			...
			*out_result = false; /* fail */
		}
	}
}

static float mean_diff_textures(GatePipeline *p, GLuint tex_a, GLuint tex_b, GLuint mask_a, GLuint mask_b)
{
	...
	glBindTexture(GL_TEXTURE_2D, mask_a);
	glActiveTexture(GL_TEXTURE3);
	glBindTexture(GL_TEXTURE_2D, mask_b);
...
static void gate_run_stability(GatePipeline *p, bool *out_result, float *out_stability_diff)
{
	/* ── Pass 3: frame diff ──────────────────────────────────────────── */
	if (!p->has_prev) {
	...
	if (!(p->cfg.diff_threshold > 0.0f)) {
		blog(LOG_DEBUG, TAG "Diff threshold disabled, skipping diff");
		return;
	}

	blog(LOG_DEBUG, TAG "Pass 3: frame diff");
	GLuint mask_cur = ((p->cfg.modes & LAP_MASK_BIT) != 0) ? p->tex_mask_dil : p->tex_mask_cur;

	float mean_diff = mean_diff_textures(p, p->tex_gray_cur, p->tex_gray_prev, mask_cur, p->tex_mask_prev);

	if (out_stability_diff)
		*out_stability_diff = mean_diff;
	// fail if frame became unstable or if not enough stable frames yet
	if (mean_diff >= p->norm_diff_thr) {
		p->stability_frames = 0;
		*out_result = false; /* fail */
		blog(LOG_DEBUG, TAG "frame unstable (diff %.6f above threshold %.6f), resetting stability_frames to 0",
...
static void gate_run_change(GatePipeline *p, bool *out_result, float *out_change_diff)
{
	/* ── Pass 4: OCR-diff ───────────────────── */
	if (!p->has_ocr) {
		blog(LOG_DEBUG, TAG "No OCR frame, skipping change gate");
		return;
	}
	if (!(p->cfg.change_diff_threshold > 0.0f)) {
		blog(LOG_DEBUG, TAG "Change diff threshold disabled, skipping change gate");
		return;
	}
	blog(LOG_DEBUG, TAG "Pass 4: OCR change diff");
	GLuint mask_cur = ((p->cfg.modes & LAP_MASK_BIT) != 0) ? p->tex_mask_dil : p->tex_mask_cur;
	float mean_diff = mean_diff_textures(p, p->tex_gray_cur, p->tex_gray_ocr, mask_cur, p->tex_mask_ocr);

	if (out_change_diff)
		*out_change_diff = mean_diff;

	if (mean_diff < p->norm_change_thr) {
		*out_result = false; /* fail */
		blog(LOG_DEBUG,
		     TAG
		     "frame change sincle last OCR below threshold (diff %.6f below threshold %.6f), failing change gate",
		     mean_diff, p->norm_change_thr);
	} else {
		blog(LOG_DEBUG,
		     TAG
		     "frame change since last OCR above threshold (diff %.6f above threshold %.6f), passing change gate",
		     mean_diff, p->norm_change_thr);
	}
}
...
{
	blog(LOG_INFO, TAG "creating pipeline for %ux%u", w, h);
	...
	p->norm_change_thr = cfg->change_diff_threshold / 255.0f;
	...
	p->tex_gray_ocr = make_tex((GLsizei)w, (GLsizei)h, GL_R32F);
	...
	p->tex_mask_ocr = make_tex((GLsizei)w, (GLsizei)h, GL_R8);
	{
	...
	p->has_prev = false;
	blog(LOG_INFO, TAG "pipeline created %ux%u num_wg=%u", w, h, p->num_wg);
	return p;
}
...
void gate_destroy(GatePipeline *p)
{
	if (!p)
		return;
	{
		GLuint texs[] = {p->tex_gray_cur, p->tex_gray_prev, p->tex_lap,      p->tex_gray_ocr,
				 p->tex_mask_cur, p->tex_mask_prev, p->tex_mask_dil, p->tex_mask_ocr};
	      ...
	      float *out_change_diff)
{
	...
	gate_run_lap(p, gl_tex_id);
	if ((p->cfg.modes & LAP_MASK_BIT) != 0) {
		gate_run_lap_carve(p);
	}
	gate_run_lap_reduce(p, &result, out_lap_var);
	if (result) {
		gate_run_stability(p, &result, out_stability_diff);
	}
	if (result) {
		gate_run_change(p, &result, out_change_diff);
	}
	...
	blog(LOG_DEBUG, TAG "lap_var=%.6f stability diff=%.6f change diff=%.6f result=%s",
	     out_lap_var ? *out_lap_var : 0.0f, out_stability_diff ? *out_stability_diff : 0.0f,
	     out_change_diff ? *out_change_diff : 0.0f, result ? "pass" : "fail");
	return result;
}