...
invoke_formatter() {
  ...
  case ${formatter} {
    clang)
      ...
      if ! is-at-least 22.1.1 ${formatter_version[-1]}; then
        log_error "clang-format is not version 22.1.1 or above (found ${formatter_version[-1]}."
        exit 2
      fi

      if ! is-at-least ${formatter_version[-1]} 22.1.1; then
        log_error "clang-format is more recent than version 22.1.1 (found ${formatter_version[-1]})."
        exit 2
      fi
